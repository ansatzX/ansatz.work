...
# Install perl, curl (required for TinyTeX) and install TinyTeX for TikZ support
RUN apt-get update && \
    apt-get install -y perl curl wget && \
    apt-get clean && rm -rf /var/lib/apt/lists/* && \
    export OSNAME=Linux && \
    export OSTYPE=linux-gnu && \
    mkdir -p /root/.local/bin && \
    wget -qO- "https://yihui.org/tinytex/install-bin-unix.sh" | sh && \
    tlmgr install dvisvgm pgf siunitx preview standalone fp && \
    npm ci && npm cache clean --force

# Copy application code and built files from build stage
